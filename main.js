...
var author$project$Main$Model = F3(
	function (inputValue, parseResult, commands) {
		return {commands: commands, inputValue: inputValue, parseResult: parseResult};
	});
var author$project$Main$initInput = 'repeat 20 [ repeat 180 [fd 5 rt 2 ] rt 18 ]';
...
var author$project$Main$parseResultToCommands = function (result) {
	if (result.$ === 'Ok') {
		var commands = result.a;
		return commands;
	} else {
		return _List_Nil;
	}
};
var author$project$Main$Backward = function (a) {
	return {$: 'Backward', a: a};
};
...
var author$project$Main$PenDown = {$: 'PenDown'};
var author$project$Main$PenUp = {$: 'PenUp'};
...
var author$project$Main$parseCommandStep = function (revStmts) {
	return elm$parser$Parser$oneOf(
		_List_fromArray(
			[
				...
				author$project$Main$cyclic$parseCommand()),
				A2(
				elm$parser$Parser$map,
				function (_n0) {
...
function author$project$Main$cyclic$parseCommand() {
	return A2(
			...
			_List_fromArray(
				[
					...
					A2(
						...
						A2(
							elm$parser$Parser$ignorer,
							elm$parser$Parser$succeed(author$project$Main$Backward),
							elm$parser$Parser$token('bk')),
						elm$parser$Parser$spaces),
					elm$parser$Parser$int),
					A2(
					elm$parser$Parser$keeper,
					...
					A2(
						...
						A2(
							...
							elm$parser$Parser$token('lt')),
						elm$parser$Parser$spaces),
					elm$parser$Parser$float),
					A2(
					elm$parser$Parser$ignorer,
					elm$parser$Parser$succeed(author$project$Main$PenUp),
					elm$parser$Parser$token('pu')),
					A2(
					elm$parser$Parser$ignorer,
					elm$parser$Parser$succeed(author$project$Main$PenDown),
					elm$parser$Parser$token('pd')),
					A2(
					elm$parser$Parser$keeper,
					A2(
						elm$parser$Parser$keeper,
					...
					A2(
						...
						A2(
							elm$parser$Parser$ignorer,
							author$project$Main$cyclic$parseInput(),
							elm$parser$Parser$spaces),
						elm$parser$Parser$symbol(']')))
				])),
		elm$parser$Parser$spaces);
}
function author$project$Main$cyclic$parseInput() {
	return A2(elm$parser$Parser$loop, _List_Nil, author$project$Main$parseCommandStep);
}
try {
	var author$project$Main$parseCommand = author$project$Main$cyclic$parseCommand();
	author$project$Main$cyclic$parseCommand = function () {
		return author$project$Main$parseCommand;
	};
	var author$project$Main$parseInput = author$project$Main$cyclic$parseInput();
	author$project$Main$cyclic$parseInput = function () {
		return author$project$Main$parseInput;
	};
} catch ($) {
...
var author$project$Main$runParseInput = function (inputString) {
	return A2(
		elm$parser$Parser$run,
		author$project$Main$parseInput,
		elm$core$String$trim(
			elm$core$String$toLower(inputString)));
};
var author$project$Main$init = function () {
	var initialResult = author$project$Main$runParseInput(author$project$Main$initInput);
	var initialCommands = author$project$Main$parseResultToCommands(initialResult);
	return A3(author$project$Main$Model, author$project$Main$initInput, initialResult, initialCommands);
}();
var author$project$Main$update = F2(
	function (msg, model) {
		var newInput = msg.a;
		var result = author$project$Main$runParseInput(newInput);
		var commands = author$project$Main$parseResultToCommands(result);
		return _Utils_update(
			model,
			{commands: commands, inputValue: newInput, parseResult: result});
	});
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A3(avh4$elm_color$Color$rgb, 0.85, 0, 0))
				]),
			_List_fromArray(
				[
					A3(
...
var author$project$Main$expandRepeats = function (commands) {
	return elm$core$List$concat(
		A2(
			...
			function (c) {
				if (c.$ === 'Repeat') {
					var count = c.a;
					var repeatCommands = c.b;
					return elm$core$List$concat(
						A2(
							elm$core$List$map,
							author$project$Main$expandRepeats,
							A2(elm$core$List$repeat, count, repeatCommands)));
				} else {
					return elm$core$List$singleton(c);
				}
			},
			commands));
};
...
var author$project$Main$nextLocation = F2(
	function (location, command) {
		var radFacing = elm$core$Basics$degrees(location.facing);
		var _n0 = location.point;
		var x = _n0.a;
		var y = _n0.b;
		switch (command.$) {
			case 'Forward':
				var amount = command.a;
				var newY = y + (elm$core$Basics$sin(radFacing) * amount);
				var newX = x + (elm$core$Basics$cos(radFacing) * amount);
				return _Utils_update(
					location,
					{
						point: _Utils_Tuple2(newX, newY)
					});
			case 'Backward':
				var amount = command.a;
				var newY = y + (elm$core$Basics$sin(radFacing + elm$core$Basics$pi) * amount);
				var newX = x + (elm$core$Basics$cos(radFacing + elm$core$Basics$pi) * amount);
				return _Utils_update(
					location,
					{
						point: _Utils_Tuple2(newX, newY)
...
var author$project$Main$generateStep = F2(
	function (command, state) {
		var newLocation = A2(author$project$Main$nextLocation, state.location, command);
		var lineFn = state.location.down ? joakin$elm_canvas$Canvas$lineTo : joakin$elm_canvas$Canvas$moveTo;
		return _Utils_update(
			state,
			{
				...
				segments: A2(
					elm$core$List$cons,
					lineFn(newLocation.point),
					state.segments)
			});
	});
var author$project$Main$initSegmentsState = function (startPoint) {
	return {
		location: {down: true, facing: 0, point: startPoint},
		segments: _List_Nil
	};
};
var author$project$Main$generatePathSegments = F2(
	function (startPoint, commands) {
		return elm$core$List$reverse(
			A3(
				elm$core$List$foldl,
				author$project$Main$generateStep,
				author$project$Main$initSegmentsState(startPoint),
				author$project$Main$expandRepeats(commands)).segments);
	});
...
var author$project$Main$showCanvasContent = F2(
	function (startPoint, commands) {
		return A2(
			joakin$elm_canvas$Canvas$shapes,
			_List_fromArray(
				[
					...
					A4(avh4$elm_color$Color$rgba, 0.85, 0.85, 0, 1)),
			...
			_List_fromArray(
				[
					...
					startPoint,
					A2(author$project$Main$generatePathSegments, startPoint, commands))
				]));
	});
...
var author$project$Main$showDrawing = function (commands) {
	var width = 750;
	var height = 750;
	var startPoint = _Utils_Tuple2(width / 2, height / 2);
	return A3(
		...
		_List_fromArray(
			[
				A2(author$project$Main$clearCanvas, width, height),
				A2(author$project$Main$showCanvasContent, startPoint, commands)
			]));
};
var author$project$Main$commandToString = function (command) {
	switch (command.$) {
		case 'Forward':
			var amount = command.a;
			return 'forward ' + elm$core$String$fromInt(amount);
		case 'Backward':
			var amount = command.a;
			return 'backward ' + elm$core$String$fromInt(amount);
		case 'Right':
			var amount = command.a;
			return 'turn right ' + elm$core$String$fromFloat(amount);
		case 'Left':
			var amount = command.a;
			return 'turn left ' + elm$core$String$fromFloat(amount);
		case 'PenUp':
			return 'pen up';
		case 'PenDown':
			return 'pen down';
		default:
			var count = command.a;
			return 'repeat ' + (elm$core$String$fromInt(count) + ':');
	}
};
var elm$html$Html$p = _VirtualDom_node('p');
var elm$virtual_dom$VirtualDom$text = _VirtualDom_text;
var elm$html$Html$text = elm$virtual_dom$VirtualDom$text;
var author$project$Main$commandToHtml = function (command) {
	return A2(
		elm$html$Html$p,
		_List_Nil,
		_List_fromArray(
			[
				elm$html$Html$text(
				author$project$Main$commandToString(command))
			]));
};
var elm$html$Html$div = _VirtualDom_node('div');
var author$project$Main$showCommand = function (command) {
	if (command.$ === 'Repeat') {
		var count = command.a;
		var commands = command.b;
		return A2(
			elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
				[
					author$project$Main$commandToHtml(command),
					A2(
					elm$html$Html$div,
					_List_fromArray(
						[
					...
					A2(elm$core$List$map, author$project$Main$showCommand, commands))
				]));
	} else {
		var c = command;
		return author$project$Main$commandToHtml(c);
	}
};
var author$project$Main$showParsedCommands = function (commands) {
	return A2(
		elm$html$Html$div,
		_List_Nil,
		A2(elm$core$List$map, author$project$Main$showCommand, commands));
};
var author$project$Main$showParseResults = function (result) {
	if (result.$ === 'Ok') {
		var commands = result.a;
		return author$project$Main$showParsedCommands(commands);
	} else {
		...
		return A2(
			...
			_List_fromArray(
				[
					elm$html$Html$text('could not build all commands')
				]));
	}
};
...
var author$project$Main$showTextArea = function (model) {
	return A2(
		elm$html$Html$textarea,
		_List_fromArray(
			[
				elm$html$Html$Attributes$placeholder('Give the turtle some commands'),
...
var author$project$Main$view = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				author$project$Main$showTextArea(model),
				author$project$Main$showDrawing(model.commands),
				author$project$Main$showParseResults(model.parseResult)
			]));
};
